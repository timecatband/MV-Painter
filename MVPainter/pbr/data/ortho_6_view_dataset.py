...
class CustomMVDataset(Dataset):
    ...
    def load_image(self, img_path, bg_color, alpha, return_type='np'):
        ...
        normal_img = Image.open(img_path.replace('result_','').replace('.png','_normal.png'))
        normal_img = normal_img.convert('RGBA')
        normal_img_arr = np.array(normal_img)

        normal_img_arr_alpha = (normal_img_arr[:,:,0] == 127) & (normal_img_arr[:,:,1] == 127) & (normal_img_arr[:,:,2] == 127)
        normal_img_arr_alpha = ~normal_img_arr_alpha
        # 使用normal作为抠图
        img = img.convert('RGBA')

        img_arr = np.array(img)
        img_arr[:,:,-1] = 0
        img_arr[normal_img_arr_alpha,-1] = 255
    ...
    def __getitem__(self, index):
        ...
        cfg = json.load(open('./pbr/data/transforms.json'))
        frames = cfg['frames']
