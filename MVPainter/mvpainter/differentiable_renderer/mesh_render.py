...
class MeshRender():
    ...
    def set_mesh_mvadapter(
        self,
        vtx_pos, pos_idx,
        vtx_uv=None, uv_idx=None,
        scale_factor=1.15, auto_center=True
    ):
        print("用于adapter set_mesh_mvadapter, 在mesh render里")
    ...
    def uv_inpaint(self, texture, mask):
        ...
        return texture_np
    def uv_inpaint_vroni(self,texture,mask):
        if isinstance(texture, torch.Tensor):
            texture_t = texture
        elif isinstance(texture, np.ndarray):
            texture_t = torch.from_numpy(texture).cuda()
        elif isinstance(texture, Image.Image):
            texture_t = torch.from_numpy(np.array(texture) / 255.0)

        mask = torch.from_numpy(mask).cuda()

        texture_t_new = voronoi_solve(texture_t,mask)
        texture_np = texture_t_new.cpu().numpy()
        texture_np = texture_np * 255
        return texture_np
